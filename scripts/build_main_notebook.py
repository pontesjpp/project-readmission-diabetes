"""Constrói main.ipynb com a estrutura das 10 seções do workflow obrigatório.

As seções 1-5 vêm preenchidas para o checkpoint (20/05/2026).
As seções 6-10 ficam como esqueleto com instruções e placeholders.

Rode com: uv run python scripts/build_main_notebook.py
"""
...
CELLS.append(md(
    ...
    "Este notebook executa o workflow obrigatório de ponta a ponta. As seções seguem "
    "a ordem exigida pelo `docs/exigencias.pdf` (Seção 7). **O teste é tocado APENAS na Seção 9**, "
    "via `load_test(unlock_token='I_AM_IN_FINAL_EVALUATION')`.\n\n"
    "Reprodução completa:\n"
    "```bash\n"
    "uv sync\n"
...
# Seção 1: Setup
# ──────────────────────────────────────────────────────────────────────
CELLS.append(md("## 1. Setup e imports"))
CELLS.append(code(
    "from __future__ import annotations\n"
    "\n"
    "import warnings\n"
...
CELLS.append(md(
    ...
    "O split é materializado uma única vez por `src/data_loader.py` "
    "(estratificado, `random_state=42`, `test_size=0.2`) e salvo em `data/interim/`. "
    "A partir daqui o teste fica isolado por trás da guarda do token.\n\n"
    "**O briefing do cliente fictício** será preenchido no relatório (template em "
    "`docs/exigencias.pdf` Seção 2.2). Resumo do cenário: hospital quer prever "
    "readmissão precoce (<30 dias) para acionar acompanhamento pós-alta; falso "
    "negativo na classe `<30` tem custo alto."
))
CELLS.append(code(
...
# ──────────────────────────────────────────────────────────────────────
# Seção 3: EDA no treino
# ──────────────────────────────────────────────────────────────────────
CELLS.append(md(
    "## 3. EDA (somente no treino)\n\n"
    "EDA mínima para o checkpoint: distribuição de classes, missingness, "
    "distribuições de variáveis-chave. Será aprofundada no relatório."
))
CELLS.append(code(
...
# ──────────────────────────────────────────────────────────────────────
# Seção 4: Pipeline baseline
# ──────────────────────────────────────────────────────────────────────
CELLS.append(md(
    "## 4. Pipeline baseline (pré-processamento mínimo)\n\n"
    "Baseline conforme exigência Seção 7.4: limpeza estrutural + imputação + "
    "OHE + StandardScaler. **Sem** balanceamento, **sem** engenharia de "
    "características. Esses elementos entram nas variantes (Seção 6).\n\n"
    "Tudo está num `Pipeline` sklearn — o `fit` só acontece dentro de cada fold da CV "
    "(via `GridSearchCV` / `RandomizedSearchCV`), nunca no dataset inteiro."
))
CELLS.append(code(
...
CELLS.append(md(
    ...
    "Para o **checkpoint (20/05/2026)** rodamos 3 modelos: Decision Tree, "
    "Random Forest e LightGBM. Cobertura dos 10 algoritmos será fechada antes da "
    "apresentação (27/05). Os outros 7 já estão registrados em `src/models.py` "
    "(flag `enabled_for_checkpoint=False`).\n\n"
    "Cada busca:\n"
...
# ──────────────────────────────────────────────────────────────────────
# Seções 6-10: esqueleto
# ──────────────────────────────────────────────────────────────────────
CELLS.append(md(
    "## 6. Variantes do treino + nova busca HP\n\n"
    "**A ser preenchido após o checkpoint.** Cada variante deve ser um novo "
    "`build_*_pipeline()` em `src/preprocessing.py`, reaproveitando blocos do baseline:\n\n"
    "- **Balanceamento**: SMOTE / undersampling / `class_weight` (use `imblearn.Pipeline` "
    "quando precisar resamplear dentro do CV).\n"
    "- **Escalas**: trocar `StandardScaler` por `RobustScaler` / `MinMaxScaler`.\n"
    "- **Codificações**: `TargetEncoder` em colunas de alta cardinalidade "
    "(`payer_code`, `medical_specialty`); manter OHE para o resto.\n"
    "- **Engenharia de características**: derivar contagens, agrupamentos ICD-9 mais finos, etc.\n\n"
    "Para cada variante, rodar a mesma `run_search` com `tag='variante_X'` e os mesmos "
    "10 modelos. Os `fold_scores` ficam pareados (mesmos folds) por construção."
))
CELLS.append(code(
...
CELLS.append(md(
    ...
    "Para cada modelo, comparar `fold_scores[modelo]` do baseline com `fold_scores[modelo]` "
    "de cada variante usando `paired_compare()`. Tabela final com:\n"
    "- F1-macro médio por config\n"
    "- diferença vs baseline\n"
    "- p-valor Wilcoxon e paired t-test\n"
    "- decisão (manter baseline / promover variante)"
))
CELLS.append(code("print('Seção 7: a preencher após o checkpoint.')\n"))
...
CELLS.append(code("print('Seção 8: a preencher após o checkpoint.')\n"))

CELLS.append(md(
    "## 9. Avaliação final no teste (única seção que toca o teste)\n\n"
    "**Esta é a única seção que pode chamar `load_test`.** "
    "Não copie esta célula para outras seções."
))
CELLS.append(code(
...
CELLS.append(code("print('Seção 10: documentação no relatório final.')\n"))
